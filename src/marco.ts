 ...
 */
function conHojasAplicadas(doc: Document, seguir: () => void): void {
  const pendientes: HTMLLinkElement[] = [];
  doc.querySelectorAll<HTMLLinkElement>("link[rel=stylesheet]").forEach((l) => {
    if (!l.sheet) pendientes.push(l);
  });
...
function glifoDeModulo(doc: Document, key: string): HTMLElement {
...
function iconoDeTexto(doc: Document, caracter: string): HTMLElement {
...
function rotulo(doc: Document, texto: string): HTMLElement {
  const n = el(doc, "span", "riel__nombre");
  n.textContent = texto;
  return n;
}

function separador(doc: Document): HTMLElement {
  const s = el(doc, "div", "riel__separador");
  s.setAttribute("aria-hidden", "true");
  return s;
}

function itemDeModulo(doc: Document, mod: ModuloServido, propio: boolean): HTMLElement {
  ...
  if (enlazable && mod.url) nodo.setAttribute("href", mod.url);
  if (propio) nodo.setAttribute("aria-current", "true");
...
function construirRiel(
  ...
  for (const mod of verticales) caja.appendChild(itemDeModulo(doc, mod, mod.key === opciones.propio));
  if (verticales.length > 0 && horizontales.length > 0) caja.appendChild(separador(doc));
  for (const mod of horizontales) caja.appendChild(itemDeModulo(doc, mod, mod.key === opciones.propio));
  ...
  if (pie) riel.appendChild(construirPie(doc, pie));
  return riel;
}

function construirPie(doc: Document, pie: PieDelRiel): HTMLElement {
  ...
  if (pie.activa) a.setAttribute("aria-current", "page");
 ...
 */
function poblarFranja(doc: Document, franja: HTMLElement, datos: ShellServido, hubUrl: string | null): void {
  if (datos.espacio) {
  ...
  if (hubUrl) {
    ...
    const enlace = (href: string, texto: string): HTMLAnchorElement => {
      const a = el(doc, "a");
  ...
  doc.addEventListener("click", () => {
    if (!menu.hidden) alternar();
  });
  doc.addEventListener("keydown", (evento) => {
    if ((evento as KeyboardEvent).key === "Escape" && !menu.hidden) alternar();
  });
}
 ...
 */
function construirBarraDeEstado(
  doc: Document,
  saldo: SaldoServido,
  urlDeRecarga: string | null,
): HTMLElement {
  const barra = el(doc, "footer", "barra-estado");
  barra.classList.add(
    saldo.enSobregiro ? "barra-estado--sobregiro" : saldo.bajo ? "barra-estado--bajo" : "barra-estado--normal",
  );
...
function construirTabs(doc: Document, tabs: readonly TabDelModulo[], rotuloDeTabs: string): HTMLElement {
  ...
  for (const tab of tabs) {
    ...
    if (tab.activa) a.setAttribute("aria-current", "page");
...
export function montarMarco(opciones: OpcionesDelMarco): MarcoMontado {
  ...
  const esperarHojas = opciones.alAplicarseLasHojas ?? ((seguir: () => void) => conHojasAplicadas(doc, seguir));
  ...
  const listo = opciones
    ...
    .then((datos) => {
      ...
      esperarHojas(() => {
        riel.querySelectorAll<HTMLElement>("[data-riel-modulos] .glifo").forEach((g) => {
          const css = getComputedStyle(g);
          const mascara = css.maskImage || (css as unknown as { webkitMaskImage?: string }).webkitMaskImage;
          if (!mascara || mascara === "none") g.className = "glifo glifo--sin-dibujo";
        });
      });
    })
    .catch(() => {
